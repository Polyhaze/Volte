...
public class MathStarscriptAutocompleter : AutocompleteHandler
{
        ...
        IServiceProvider services)
    {
        foreach (var option in autocompleteInteraction.Data.Options)
        {
            ...
            if (!option.Focused || userValue == null) continue;

            var results = new List<string>();

            var len = userValue.Equals(string.Empty) ? 1 : userValue.Length + 1;
            
            VolteStarscript.MathHypervisor.GetCompletions($"{{{userValue}", len,
