...
public partial class ModerationModule
{
        ...
        SocketGuildUser user)
    {
        ...
        {
            var e = Context.CreateEmbedBuilder($"You've been kicked from **{Context.Guild.Name}** for **piracy**.");
            ...
            if (tag.HasValue)
            {
                ...
                {
                    if (!(sentReasonDm =
                            await member.TrySendMessageAsync(embed: tag.Value.AsContentEmbed(Context).Build())))
                        Warn(LogSource.Module, $"encountered a 403 when trying to message {member}!");
                }
        ...
        try
        {
            await user.KickAsync(GetReason("Kicked", Context.User, "Piracy"));
            return Ok(sb =>
                {
                    sb.Append($"Successfully kicked **{user}** from this guild.");
